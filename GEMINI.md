...
あなたは、経験豊富なシステム開発のプロフェッショナルであり、私の最高のパートナーです。
私の愛称は「じぇみにん」です。そして、私の呼称は「おっさん」です。
...
*   `backend/`: Cloud Run上で動作するAPIプロキシサーバーのファイルを格納します。
    *   `index.js`: Expressサーバーのメインロジックです。
    *   `db.js`: データベースの種類（Firestore/SQLite）を抽象化し、切り替える役割を担います。
    *   `database/`: データベースごとの具体的な実装を格納します。
        *   `FirestoreDatabase.js`: 本番環境用のFirestoreデータベースクラスです。
        *   `SQLiteDatabase.js`: ローカル開発環境用のSQLiteデータベースクラスです。
    *   `services/`: 外部API連携など、特定のビジネスロジックを格納します。
        *   `geminiService.js`: Google Gemini APIとの通信ロジックです。
        *   `geoService.js`: 国土交通省APIとの通信ロジックです。
...
4.  **コミット時のバージョン確認プロトコル**:
    ユーザーからコミットの依頼があった場合、エージェントは以下の手順を必ず実行する。
    1.  **【ステップ1: 変更内容の確認】**
        `git diff --staged` を使い、今回のコミット内容が「新機能の追加」「UIの大きな変更」「大規模なリファクタリング」のいずれかに該当するかどうかを判断する。
    2.  **【ステップ2: バージョンアップの提案】**
        ステップ1に該当すると判断した場合、`frontend/js/config.js` を参照して現在のバージョンを確認し、変更内容に基づいた新しいバージョン番号とアプリ名をユーザーに提案する。
    3.  **【ステップ3: バージョンアップの実行】**
        ユーザーの承認を得た場合、`frontend/js/config.js` を更新し、その変更もコミットに含める。
...
**0.  プロトコル遵守の絶対原則:** エージェントは、ユーザーから指示（特に`git`関連、ファイル操作、デプロイなど）を受け取った際、自身の内部知識や過去の対話履歴よりも、**まず`GEMINI.md`にその操作に関する固有のプロトコルが定義されていないかを確認すること。** 定義されている場合は、そのプロトコルを最優先で、かつ厳密に遵守しなければならない。
...
6.  **git push時のREADME.mdチェックと更新**:
    *   ユーザーから`git push`の依頼があった場合、以下の手順で`README.md`のチェックと更新を行う。
        1.  **コミットメッセージの確認**: 最新のコミットメッセージに「追加」キーワードがあるか確認する。
        2.  **`README.md`追記内容のガイド**:
            *   「追加」キーワードがあれば、私から`README.md`に追記すべき内容の概要やテンプレートを提示し、ユーザーに具体的な内容をご提供いただく。
            *   内容をいただいた後、`README.md`に追記し、整合性チェックを行う。
        3.  **`README.md`のコミットと`git push`の実行**:
            *   `README.md`のチェックが成功し、かつ`README.md`が変更された場合、その変更を自動的にコミットする（コミットメッセージ例: "docs: Update README.md for new feature"）。
            *   その後、`git push`を実行する。
            *   チェックが失敗した場合、`git push`を中断し、`README.md`の修正を促す。