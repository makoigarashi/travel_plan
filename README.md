...
*   **応答フォーマットの統一:** AIに特定のMarkdown形式で応答するように指示することで、GeminiやMistralなど、どのAIを利用しても比較しやすい、統一された形式で旅行プランを受け取ることができます。
*   **履歴の保存・管理:** 生成したMarkdownプロンプトをタイトル付きでデータベースに保存し、いつでも呼び出して再利用できます。保存された履歴は一覧で確認でき、フォームへの読み込みや不要な履歴の削除も可能です。
